...
sales_per_month = []    #initializing a list to store numbers of phones sold per month
...
for sale in range(months_per_year):     #using for loop 12 times(for all months per year) to get number of sales each month and calculate respective salaries for the same month
    ...
    sales_per_month.append(phones_sold)                 #storing the input in the respective list
    for sale in range(phones_sold):                     #using for loop for the number of phones sold
    ...
    if monthly_salary>max_salary:                #using if to check whether monthly salary is higher than maximum salary
        salary_per_month.append(max_salary)      #adding maximum salary to the monthly salary list
...
average_of_monthly_per_year = sum_of_monthly_salaries / months_per_year  #calculating average of monthly salary per year
